...
<td class="table-row-fixed-td" style="font-family: Arial, sans-serif; line-height: 19px; color: #444444; font-size: 13px; font-weight: normal; padding-left: 24px; padding-right: 24px;" valign="top" align="left">
   ...
   <p>Olá {{$ds_nome}},</p>
   <br/>
   <p>Seu usuário foi registrado no sistema do {{ config('app.name') }}.</p>
   <p>Para acessar o bolão entrar na URL abaixo.</p>
   <p>{{ config('app.url') }} </p>
   <p>Informe seu email - {{$ds_email}}</p>
   <p>E informe sua senha inicial - {{$ds_senha}}</p>
   <br/>
   <p>Após acessar o sistema pela primeira vez, entre em perfil e altere sua senha para uma de sua preferência.</p>
   <br/>
   <p>Atenciosamento,</p>
   <p>{{ config('app.name') }}</p>
</td></tr></tbody></table>
