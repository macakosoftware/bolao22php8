...
<td class="table-row-fixed-td" style="font-family: Arial, sans-serif; line-height: 19px; color: #444444; font-size: 13px; font-weight: normal; padding-left: 24px; padding-right: 24px;" valign="top" align="left">
   <h1>Lembrete para Aposta Não Feita</h1>
   <br/>
   <p>Olá {{$aviso->usuario->name}},</p>
   <br/>   
   <p>O(s) jogo(s) abaixo irá ocorrer em @if($aviso->tp_aviso == \App\AvisoAposta::TIPO_AVISO_24) 24 Horas @elseif ($aviso->tp_aviso == \App\AvisoAposta::TIPO_AVISO_48) 48 horas @elseif ($aviso->tp_aviso == \App\AvisoAposta::TIPO_AVISO_72) 72 Horas @endif e ainda não temos registrado seu palpite.
   Por favor logue-se em {{ config('app.url') }} e registre seu palpite, pois uma hora antes do jogo as apostas serão encerradas.
   <h1>{{\Carbon\Carbon::parse($aviso->jogo->dt_jogo)->format('d/m/Y')}} - {{substr($aviso->jogo->hr_jogo, 0, 5)}} 
   <img src="{{ asset('images/brasoes') }}/{{$aviso->jogo->selecao1->ds_icone}}" /> {{$aviso->jogo->selecao1->ds_nome}} x 
   <img src="{{ asset('images/brasoes') }}/{{$aviso->jogo->selecao2->ds_icone}}" /> {{$aviso->jogo->selecao2->ds_nome}}</h1>
