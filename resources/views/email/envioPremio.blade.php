...
<td class="table-row-fixed-td" style="font-family: Arial, sans-serif; line-height: 19px; color: #444444; font-size: 13px; font-weight: normal; padding-left: 24px; padding-right: 24px;" valign="top" align="left">
   ...
   <p>Olá,</p>
   <br/>   
   <p>O {{ config('app.name')}} gostaria de parabenizar {{$premio->usuario->name}} pelo prêmio recebido em virtude do {{$premio->nr_posicao}}o. lugar no Ranking da {{$premio->tipoRanking->ds_nome}}.</p>
   <p>Estamos enviando em anexo o comprovante do pagamento do seu prêmio no valor de {{number_format($premio->vl_premio, 2, ',', '.')}}.</p>
   <br/>
   <p>Atenciosamento,</p>
   <p>{{ config('app.name') }}</p>
</td></tr></tbody></table>
